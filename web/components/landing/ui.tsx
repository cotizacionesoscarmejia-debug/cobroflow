...
/* ── <Perforacion> — dispositivo ownable de FICHA-ARTE.md: "recibo perforado".
   Franja de perforación de ticket en el borde de un elemento (cards de precio,
   el placeholder del hero) — el gesto que hace que CobroFlow no sea intercambiable
   con otro proyecto que use el mismo kit. Requiere un contenedor `relative`. ── */
export function Perforacion({ className = '', tono = 'accent' }: { className?: string; tono?: 'accent' | 'neutro' }) {
  return (
    <span
      aria-hidden="true"
      className={`absolute inset-y-4 left-1.5 w-[3px] rounded-full ${className}`}
      style={{
        backgroundImage: `repeating-linear-gradient(180deg, var(${tono === 'accent' ? '--accent' : '--text-tertiary'}) 0 8px, transparent 8px 16px)`,
      }}
    />
  );
}
